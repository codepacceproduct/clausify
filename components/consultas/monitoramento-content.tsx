...
import { deleteMonitoredProcess, updateMonitoringStatus, getMonitoredProcesses } from "@/actions/monitoring"
import { toast } from "sonner"

interface MonitoramentoContentProps {
  initialProcesses: MonitoredProcess[]
}

export function MonitoramentoContent({ initialProcesses }: MonitoramentoContentProps) {
  const [viewState, setViewState] = useState<"list" | "add">("list")
  const [processes, setProcesses] = useState<MonitoredProcess[]>(initialProcesses)
  ...
  // We don't need useEffect to load initial processes since we have them from server
  // But we might want to refresh if the user navigates back to this page or based on some trigger
  // For now, we trust initialProcesses.
  
  // If the user adds a process, we reload.
  ...
  const handleStatusChange = async (id: string, status: MonitoredProcess["status"]) => {
    ...
    if (!result.success) {
      // Revert if failed
      await loadProcesses() // Reload from server to be safe
